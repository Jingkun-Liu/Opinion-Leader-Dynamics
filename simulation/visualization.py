...
MODEL_TITLES = {
    "standard": "Standard Attention Dynamics",
    "explicit": "Explicit Opinion Leader Dynamics",
    "implicit": "Implicit Opinion Leader Dynamics",
}
...
def model_subtitle(result: dict) -> str:
    ...
    if result["model"] == "standard":
        if config.standard_init == "l2":
            return (
                f"n={config.n}, beta={config.beta:g}, initialization=L2-vMF, "
                f"kappa={config.standard_l2_kappa:g}"
            )
        return f"n={config.n}, beta={config.beta:g}, initialization={config.standard_init}"
    if result["model"] == "explicit":
    ...
    gaps = result["metadata"]["graph"].spectral_gaps
    gap_text = ", ".join(f"{value:.3f}" for value in gaps)
    return (
        f"n={config.n}, beta={config.beta:g}, components={config.components}, "
        f"R={config.interaction_radius:g}, spectral gaps=[{gap_text}]"
    )
